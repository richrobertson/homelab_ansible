...
| **Nextcloud** `nextcloud` ns (`nextcloud-migration-ldap`, prod, 3 replicas) | As above, but `ldapBase=dc=myrobertson,dc=net` (domain root). | Read on user and group objects. | **Orphan Secret, no Vault source.** | As above. |
